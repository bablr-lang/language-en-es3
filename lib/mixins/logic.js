...
import { buildPattern, buildRegexGroup } from '@bablr/helpers/builders';
import {
  m,
  o,
  defineAttribute,
...
import { freezeClass, deepFreezeRecord } from '@bablr/agast-helpers/object';
...
const buildOperatorAlternatives = (categories) =>
  deepFreezeRecord([...flatMap((operators) => arrayValues(operators), arrayValues(categories))]);

export const assignmentOperators = deepFreezeRecord([
  '=',
  '+=',
  '-=',
  '*=',
...
export const unaryPrefixOperators = deepFreezeRecord([
  'typeof',
  'void',
  'delete',
  '++',
...
export const unaryPostfixOperators = deepFreezeRecord(['++', '--']);
export const unaryPostfixOperatorAlternatives = buildOperatorAlternatives([unaryPostfixOperators]);

const binaryExpressionPowerRank = deepFreezeRecord([
  ['||'],
  ['&&'],
  ['|'],
  ['^'],
...
let assignables = deepFreezeRecord(['MemberExpression', 'Identifier']);

const mixin = (Base) => {
  class ES3LogicGrammar extends Base {
    static context = deepFreezeRecord({ ...super.context, assignables, powers });
    ...
    *LogicExpression({ language, s, props: { power, noIn = false } }) {
      ...
      matcher ??= m`/[[.(?,]|===?|${printSource(
        buildRegexGroup(unaryPostfixOperatorAlternatives),
      )}|${printSource(buildRegexGroup(assignmentOperatorAlternatives))}|${printSource(
        buildRegexGroup(getBinaryOperatorAlternatives(p, power_)),
      )}/`;
    ...
    *UnaryExpression({ literalValue, props: { power, op = printSource(literalValue) }, s }) {
      ...
      if (!s().shifted) {
        pre = yield eat(
          m`sigilToken*: <*${op && /[a-z]/.test(op[0]) ? 'Keyword' : ''} ${printSource(
      ...
      if (!pre) {
        yield eat(
          m`sigilToken*: <* ${printSource(buildPattern(unaryPostfixOperatorAlternatives))} />`,
        );
    ...
    *BinaryExpression({ props: { op, power, noIn } }) {
      ...
      yield eat(
        m`sigilToken*: <*${['instanceof', 'in'].includes(op) ? 'Keyword' : ''} ${printSource(
          pattern,
        )} />`,
      );
    ...
    *AssignmentExpression() {
      ...
      assignmentMatcher ??= m`assignmentOperator*: <* ${printSource(
        buildPattern(assignmentOperatorAlternatives),
      )} />`;

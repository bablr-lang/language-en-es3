...
let assignables = freeze(['MemberExpression', 'Identifier']);

const mixin = (Base) => {
  class ES3LogicGrammar extends Base {
    static context = immutable({ ...super.context, assignables, powers });
    ...
    *LogicExpression({ language, s, props: { power, noIn = false } }) {
      ...
      if (!res && power_ >= p.assign) {
        if (
          language.context.assignables.includes(getRoot(shifted).value.name.description) &&
  ...
  freeze(ES3LogicGrammar);
  freeze(ES3LogicGrammar.prototype);

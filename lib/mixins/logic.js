...
import { printSource as printStreamSource, printString } from '@bablr/agast-helpers/stream';
...
export const getBinaryOperatorAlternatives = (powers, power) => {
  return buildOperatorAlternatives(
    arraySlice(binaryExpressionPowerRank, Math.max(0, powers.logical_or - power)),
  );
};

const getBinaryOperatorPower = (p, str) => {
  let idx = arrayFindIndex(binaryExpressionPowerRank, (arr) => arrayIncludes(arr, str));
  ...
  class ES3LogicGrammar extends Base {
    ...
    constructor() {
      ...
      this.attributes = BMap.concat(
        ...
        BMap.from(entry('UnaryExpression', freezeRecord({ position: undefined }))),
      );
    }
    ...
    *LogicExpression({ language, s, props: { power, noIn = false } }) {
      ...
      let op = yield match(
        m`/[[.(?,]|===?|!==?|${printStreamSource(
          ...
          writeRegexGroup(assignmentOperatorAlternatives),
        )}|${printStreamSource(writeRegexGroup(getBinaryOperatorAlternatives(p, power_)))}/`,
      );
      ...
      if (!res && power_ >= p.unary_postfix && arrayIncludes(unaryPostfixOperators, op)) {
        res = yield eat(m`<UnaryExpression />`, o({ op, power: power_ }));
      }
      if (!res && power_ >= p.multiply) {
        for (let powerRank of [...arrayValues(binaryExpressionPowerRank)].slice(
          Math.max(0, p.logical_or - power_),
        )) {
          if (arrayIncludes(powerRank, op)) {
      ...
      if (!res && power_ >= p.assign) {
        if (
          arrayIncludes(language.context.assignables, getRoot(shifted).value.name.description) &&
          arrayIncludes(assignmentOperators, op) &&
    ...
    *BinaryExpression({ props: { op, power, noIn } }) {
      ...
      yield eat(
        m`sigilToken*: <*${['instanceof', 'in'].includes(op) ? 'Keyword' : ''} ${printString(
          op,
        )} />`,
      );
    ...
    *AssignmentExpression() {
      ...
      yield eat(
        m`assignmentOperator*: <* ${printStreamSource(

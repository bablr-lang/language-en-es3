...
export const mixin = (Base) =>
  class ES3LogicGrammar extends Base {
    ...
    *LogicExpression({ s, props: { power = 34, noIn = false } }) {
      ...
      )}|${buildRegexGroup(getBinaryOperatorAlternatives(power))}|${buildRegexGroup(
    ...
    *UnaryExpression({ props: { power }, s }) {
      ...
      if (!s.holding) {
        ...
        unaryPrefixMatcher ??= m`sigilToken: <*Punctuator ${buildPattern(
      ...
      if (!op) {
        unaryPostfixMatcher ??= m`sigilToken: <*Punctuator ${buildPattern(
          unaryPostfixOperatorAlternatives,
        )} />`;
    ...
    *BinaryExpression({ props: { power, noIn }, ctx }) {
      ...
      matcher ??= m`sigilToken: <*Punctuator ${buildPattern(
      ...
      let opTxt = ctx.sourceTextFor(op.node).trim();
    ...
    *AssignmentExpression() {
      ...
      if (!['MemberExpression', 'Identifier'].includes(getRoot(lhs.node.node).type.description)) {
        yield fail();
      }
      assignmentMatcher ??= m`assignmentOperator: <*Punctuator ${buildPattern(
        assignmentOperatorAlternatives,
      )} />`;

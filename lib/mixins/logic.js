...
import { defineAttribute, eat, eatHeld, eatMatch, fail, match, o } from '@bablr/helpers/grammar';
...
const mixin = (Base) =>
  class ES3LogicGrammar extends Base {
    ...
    *LogicExpression({ s, props: { power = 34, noIn = false } }) {
      ...
      op = op.value.children[1][0]?.value;
    ...
    *UnaryExpression({ literalValue, props: { power, op = literalValue }, s }) {
      let ownPower = 0;
      let pre;
      if (!s().holding) {
        pre = yield eatMatch(
    ...
    *AssignmentExpression({ s }) {
      if (!['MemberExpression', 'Identifier'].includes(getRoot(s().held).value.name.description)) {
        yield fail();
      }

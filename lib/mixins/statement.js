...
let getTrailingTrivia = (node, getGapNode) => {
  let trailingBounds = node.value.bounds[1];
  debugger;
  let frame;
  for (let i = BTree.getSize(trailingBounds) - 1; i >= 0; i--) {
    frame = BTree.getAt(-3, trailingBounds);
    if (frame.property.value.reference) break;
  }
  return frame.property.value.node;
};
  ...
  class ES3StatementGrammar extends Base {
    ...
    *StatementList({ ctx, matcher }) {
      ...
      while ((chr = yield match(re`/[^;]|\g/s`))) {
        ...
        trivia = sep ? true : getTrailingTrivia(stmt.node, getGapNode);

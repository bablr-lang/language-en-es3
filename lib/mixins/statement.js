  ...
  class ES3StatementGrammar extends Base {
    ...
    *StatementList({ ctx, s, matcher }) {
      ...
      while (yield match(re`/./s`)) {
        ...
        if (noSemiStatements.includes(stmt.node.type) && !(stmt.node.get('endToken') || newline))
          break;
      }
    }
    ...
    *TryCatchStatement() {
      ...
      let catch_ = yield eatMatch(m`handler*: <CatchClause 'catch' />`, o({}), o({ bind: true }));
      let finalizer = yield eatMatch(
        m`finalizer*: <FinallyClause 'finally' />`,
        o({}),
        o({ bind: true }),
      );

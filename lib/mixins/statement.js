 ...
 * @generatedfrom ./statement.macro.js#ed38e8b9a9b1bb0e3a8f9202f6714c71158471b8
...
export const mixin = Base => {
  ...
  return class ES3StatementGrammar extends Base {
    ...
    *Statement({
      ...
      let res;
      if (s.source.done) {
        yield eat(m`<EmptyStatement />`, o({
          allowEmpty
        }));
      } else if (res = yield match(re`/;|\{|(?:if|while|do|switch|function|try|return|continue|break|throw|debugger|var)\b/`)) {
        let productions = {
          ';': m`<EmptyStatement />`,
          '{': m`<BlockStatement  />`,
          if: m`<IfStatement />`,
      ...
      } else if (yield eatMatch(m`<ExpressionStatement />`)) {} else if (yield eatMatch(m`<LabeledStatement />`)) {}
    }
    *EmptyStatement({
      props: {
        allowEmpty = true

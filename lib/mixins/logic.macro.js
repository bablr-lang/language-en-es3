...
export const mixin = (Base) =>
  class ES3LogicGrammar extends Base {
    *LogicExpression({ props: { power = 34, noIn = false } }) {
      let res;
      if (power >= 2) {
        res = yield eatMatch(m`<MemberExpression /\g[[.]/ />`, o({ power }));
      }
      if (!res && power >= 6) {
        res = yield eatMatch(m`<CallExpression /\g\(/ />`, o({ power }));
      }
      if (!res && power >= 8) {
        res = yield eatMatch(
          m`<UnaryExpression /\g${buildRegexGroup(unaryPostfixOperatorAlternatives)}/ />`,
          o({ power }),
        );
      }
      if (!res && power >= 12) {
        res = yield eatMatch(
          m`<BinaryExpression /\g${buildRegexGroup(getBinaryOperatorAlternatives(power))}/ />`,
          o({ power, noIn }),
        );
      }
      if (!res && power >= 32) {
        if ((res = yield eatMatch(m`<AssignmentExpression />`, o({ power })))) {
        } else if ((res = yield eatMatch(m`<TernaryExpression /\g\?/ />`, o({ power })))) {
        }
      }
      if (!res && power >= 34) {
        res = yield eatMatch(m`<SequenceExpression /\g,/ />`, o({ power }));
      }
    }
    ...
    *MemberExpression({ ctx }) {
    ...
    @Node
    *TernaryExpression() {
      yield eat(m`test+$: <_Expression />`, o({ power: 30 }));
      yield defineAttribute('power', 32);

      yield eat(m`consequentSigilToken: <*Punctuator '?' />`);
      yield eat(m`consequent+$: <_Expression />`, o({ power: 30 }));
      yield eat(m`alternateSigilToken: <*Punctuator ':' />`);
      yield eat(m`alternate+$: <_Expression />`, o({ power: 30 }));
    }

...
import { spam as m, buildTag } from '@bablr/boot';
import * as regexLang from '@bablr/boot/languages/regex';
import { bindAttribute, eat, eatMatch, fail, o } from '@bablr/helpers/grammar';
import { UnboundAttributes, Node, CoveredBy } from '@bablr/helpers/decorators';
...
export const mixin = (Base) =>
  class ES3LogicGrammar extends Base {
    *LogicExpression({ value: props, ctx }) {
      const { power = 34 } = props?.value || {};
      let res;
      if (power >= 34) {
    ...
    @Node
    *UnaryExpression({ value: props }) {
      const { power } = props?.value || {};
      let ownPower = 0;
      let op = yield eat(m`sigilToken: <*Punctuator ${unaryPrefixOperatorPattern} />`);
      if (op) ownPower = 10;
      yield eat(m`argument+$: <Expression />`);
      ...
      yield bindAttribute('power', ownPower);
    }
    ...
    @CoveredBy('LogicExpression')
    @Node
    *BinaryExpression({ value: props, ctx }) {
      let { power } = props?.value || {};
      yield eat(m`left+$: <Expression />`, o({ power: power - 2 }));
      ...
      yield eat(m`right+$: <Expression />`, o({ power: ownPower - 2 }));
    ...
    *AssignmentExpression() {
      yield bindAttribute('power', 32);
      let lhs = yield eat(m`left+$: <Expression />`, o({ power: 2 }));
      if (!['MemberExpression', 'Identifier'].includes(lhs.get('.').type.description)) {
        yield fail();
      }
      yield eat(m`sigilToken: <*Punctuator ${assignmentOperatorPattern} />`);
      yield eat(m`right+$: <Expression />`, o({ power: 30 }));
    }
    ...
    *MemberExpression() {
      ...
      yield eat(m`object+$: <Expression />`, o({ power: 2 }));
      yield eat(m`sigilToken: <*Punctuator '.' />`);
      yield eat(m`property+$: <Expression />`, o({ power: 2 }));
    }
    ...
    *TernaryExpression() {
      yield bindAttribute('power', 32);
      yield eat(m`test+$: <Expression />`, o({ power: 30 }));

      yield eat(m`consequentSigilToken: <*Punctuator '?' />`);
      yield eat(m`consequent+$: <Expression />`, o({ power: 30 }));
      yield eat(m`alternateSigilToken: <*Punctuator ':' />`);
      yield eat(m`alternate+$: <Expression />`, o({ power: 30 }));
    }
    ...
    @Node
    *CommaExpression() {
      yield bindAttribute('power', 34);

      let count = 0;

      do {
        yield eatMatch(m`elements[]+$: <Expression />`, o({ power: 32 }));
        count++;

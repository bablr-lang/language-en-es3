...
import { eat, match, eatMatch, o, holdForMatch, fail } from '@bablr/helpers/grammar';
...
export const atrivialGrammar = class ES3Grammar extends functionMixin(
  ...
  *Expression({ value: props, s }) {
    const { power = 34 } = props?.value || {};
    let res;
    if (!s.holding) {
      if ((res = yield eatMatch(m`<JSONExpression />`))) {
      } else if (
        power >= 4 &&
      ...
      res = yield eatMatch(m`<LogicExpression />`, o({ power }));
    }
    if (res) {
      return holdForMatch(m`<Expression />`, o({ power }));
    }
  }
  ...
  @Node
  *Identifier({ ctx }) {
    let id = ctx.sourceTextFor(yield eat(re`/[a-zA-Z_$][a-zA-Z\d_$]*/`));

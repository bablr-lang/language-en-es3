...
export const grammar = class RegexGrammar {
  ...
  constructor() {
    let { flagCharacters } = this.constructor;

    this.literals = new Set(['Keyword', 'Gap']);
    ...
    this.attributes = new Map(
      Object.entries({
        ...
        Flags: mapObject(() => undefined, flagCharacters),
        LookaheadGroup: {
          negate: undefined,
        },
  ...
  *FlatPattern() {
    ...
    outer: while ((seg = yield eat(re`/\/|[[\](\\]|[\w\d*+|^{}\u0060)$-]+/`))) {
  ...
  *Flags() {
    ...
    flagsPattern ||= re`/${buildQuantifier(
      '+',
  ...
  *EscapeSequence({ ctx, s }) {
    ...
    if ((m_ = yield match(re`/[\\/fnrtv]/`))) {
      ...
      cooked = String.fromCodePoint(
        parseInt(printSource(ctx.getGapNode(get('value', code.node))), 16),
      );
    } else {
      yield fail();
    }

...
import { printAttributes, printTag, printSource, printString } from '@bablr/agast-helpers/tree';
...
import { writeCharacterClass } from '@bablr/helpers/builders';
import { printSource as printStreamSource } from '@bablr/agast-helpers/stream';

const escapables = freezeRecord({
  f: '\f',
  n: '\n',
  r: '\r',
  t: '\t',
...
const flagCharacters = freezeRecord({
  global: 'g',
  ignoreCase: 'i',
  multiline: 'm',
});
...
export default class Regex {
  ...
  static context = freezeRecord({ flagCharacters });
  ...
  constructor() {
    ...
    this.attributes = BMap.from(
      entry('FlatPattern', freezeRecord({ groups: undefined, flags: undefined })),
      entry('Flags', freezeRecord(mapObject(() => undefined, flagCharacters))),
      entry('LookaroundGroup', freezeRecord({ negate: undefined, direction: undefined })),
  ...
  *Pattern() {
    ...
    let flatMatch = yield match(m`<*FlatPattern />`);
    yield endSpan();

    if (!flatMatch) yield fail();

    let props = flatMatch.node.value.attributes;

    yield eat(m`openToken*: <* '/' />`);
    yield startSpan('Pattern', '/', printAttributes(props));
  ...
  *Flags() {
    ...
    flagsPattern ||= m`/${printStreamSource(
      writeCharacterClass([false, ...Object.values(flagCharacters)]),
    )}+/`;
  ...
  *EscapeSequence({ ctx, s }) {
    ...
    if ((m_ = yield match(m`/[\\/fnrtv]/`))) {
      ...
      yield eat(m`code*: <*Keyword ${printString(match_)} { closeSpan: 'Escape' } />`);

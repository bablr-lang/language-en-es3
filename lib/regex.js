...
let _initProto, _FlagsDecs, _GroupDecs, _LookaheadDecs, _AssertionDecs, _StartOfInputAssertionDecs, _EndOfInputAssertionDecs, _WordBoundaryAssertionDecs, _CharacterDecs, _CharacterClassDecs, _CharacterClassRangeDecs, _CharacterSetDecs, _AnyCharacterSetDecs, _DigitCharacterSetDecs, _SpaceCharacterSetDecs, _WordCharacterSetDecs, _QuantifierDecs, _EscapeSequenceDecs, _AnyDecs, _KeywordDecs, _PunctuatorDecs;
...
export const grammar = stateEnhancer({
  ...
  *EscapeSequence({
    ...
    if (match = yield i`match(/[\\/nrt0]/)`) {
      ...
      cooked = String.fromCharCode(parseInt(codeNode.properties.digits.map(digit => ctx.sourceTextFor(digit)).join(''), type >= '0' && type <= '9' ? 8 : 16));
    } else {
      yield i`fail()`;
    }
